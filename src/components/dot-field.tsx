...
const DotField = memo(function DotField({
  ...
  sparkle = false,
  waveAmplitude = 0,
  gradientFrom = "rgba(52, 224, 164, 0.3)",
  gradientTo = "rgba(56, 100, 86, 0.2)",
  glowColor = "#0C1814",
  className,
  ...rest
}: DotFieldProps) {
  ...
  const propsRef = useRef<Omit<DotFieldProps, "className" | "glowRadius" | "glowColor">>({});
  propsRef.current = {
    dotRadius,
    dotSpacing,
    cursorRadius,
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      grad.addColorStop(0, p.gradientFrom ?? "rgba(52, 224, 164, 0.3)");
      grad.addColorStop(1, p.gradientTo ?? "rgba(56, 100, 86, 0.2)");
      ctx.fillStyle = grad;
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            ...
            const angle = Math.atan2(dy, dx);
            const move = (500 / Math.sqrt(distSq)) * (m.speed * (p.cursorForce ?? 0.1));
